...
contract OracleSlippageTest is OracleSlippage {
    mapping(address => mapping(address => mapping(uint24 => ISmartSwapPool))) private pools;
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view override returns (ISmartSwapPool pool) {
        pool = pools[tokenA][tokenB][fee];
    }

    function testGetBlockStartingAndCurrentTick(ISmartSwapPool pool)
        external
        view
